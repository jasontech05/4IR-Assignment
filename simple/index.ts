...
export const guestbookUrl = pulumi.interpolate`http://${frontendIp}`;
...
// Admin password stored as a Pulumi secret so it is masked in stack outputs.
const grafanaAdminPassword = pulumi.secret("Fun2day!");
...
const guestbookDashboardJson = JSON.stringify({
    ...
    templating: {
        list: [
            // "datasource" type variable: Grafana resolves this to the actual UID
            // at render time, so it works correctly in provisioned dashboards.
            {
                name: "datasource", type: "datasource", pluginId: "prometheus",
                label: "Datasource", hide: 0, current: {}, refresh: 1,
                query: "prometheus", options: [], multi: false, includeAll: false,
            },
            {
...
const grafana = new k8s.helm.v3.Release("grafana", {
    ...
    values: {
        adminUser: "admin",
        // Pass the plain-text value into Helm; Pulumi marks the output secret.
        adminPassword: "Fun2day!",
        service: {
            type: "LoadBalancer",
            // Expose on port 3000 so it does not clash with the frontend on :80.
            port: 3000,
            targetPort: 3000,
        },
...
// Read the Grafana LoadBalancer service to get its external IP.
// The Helm chart names the service after the release name: "grafana".
const grafanaSvc = k8s.core.v1.Service.get(
    "grafana-svc",
    pulumi.interpolate`${monitoringNamespace.metadata.name}/grafana`,
    { dependsOn: grafana },
);

const grafanaLbIp = grafanaSvc.status.loadBalancer.ingress[0].ip;

// ============================================================
// OUTPUTS
// ============================================================

export const grafanaUrl = pulumi.interpolate`http://${grafanaLbIp}:3000`;
